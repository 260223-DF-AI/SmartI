...
```sql
create table authors(authorID serial primary key, firstName varchar(50), lastName varchar(50), bio text);
CREATE TABLE orders(orderID serial primary key, foreign key customerID references customers(customerID), status varchar(20));
create table booksOrdered(foreign key isbn varchar(150) references books(isbn), foreign key orderID references orders(orderId), quantity int, primary key(isbn,orderID));
```
